...
class Vision_node:
    ...
    def __init__(self):
        ...
        self.pointcloud_subscriber = ros.Subscriber("/ur5/zed_node/point_cloud/cloud_registered", PointCloud2, self.get_point_cloud, queue_size=1)
        #TODO self.ack_subscriber = ros.Subscriber('/vision/ack', Int32, self.ackCallbak)
    ...
    def get_point_cloud(self, msg):
        if not self.processing_point_cloud: # if vision node is not already processing another point cloud
            ...
            for block in self.block_list:
                for data in point_cloud2.read_points(msg, field_names=['x','y','z'], skip_nans=True, uvs=block.center_uv):
    ...
    def send_msg(self, block):
        ...
        if len(self.msg_list) > 0:
            ...
            self.pos_publisher.publish(current_msg)
            print('\nPublished:\n', current_msg)
        else:
